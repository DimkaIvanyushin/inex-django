...
def getOfferPDF(request):
    ...
    p.drawString(300, 645, "р/с  BY76BPSB30121160110139330000")
    p.drawString(350, 630, "в ОАО «БПС-Сбербанк»")
    p.drawString(340, 615, "г. Минск, б-р Мулявина, 6  ")
    p.drawString(300, 600, "SWIFT BIC: BPSBBY2X УНП 300000252")

    p.setFont("Exo2-Medium", 16)
    ...
    for inx,product in enumerate(products):
        p.drawString(50, y, product.title[:39])
        p.drawString(320, y, str(product.count))
        p.drawString(390, y, product.group.title[:20])
        price_product = product.count * product.price
        p.drawString(480, y, str(price_product))
        y -= 25
        res_sum += price_product

    p.drawString(440, y, "Итого:")
    p.drawString(480, y, str(res_sum))
